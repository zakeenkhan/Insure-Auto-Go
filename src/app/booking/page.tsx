...
import { Booking as BookingsResponse, Booking as CarBooking } from "@/types"
import { Button } from "@/components/ui/button"
import { useGetMeQuery } from "@/lib/features/user/me-api"
import { useAllBookingsQuery } from "@/lib/features/booking/bookings-api"
import Image from "next/image"
import underline from "./underline.png"
import { useCancelSingleBookingMutation } from "@/lib/features/booking/cancel-booking-api"
import { useConfirmSingleBookingMutation } from "@/lib/features/booking/confirm-booking-api"
import { format } from "date-fns"
import { useCompleteSingleBookingMutation } from "@/lib/features/booking/complete-booking-api"
...
export default function Booking() {
  ...
  const [allBookingsData, setAllBookingsData] = useState<BookingsResponse[]>([])
  const [myBookingData, setMyBookingData] = useState<BookingsResponse[]>([])
  // rating
  ...
  useEffect(() => {
    if (!isLoading) {
      setBookings(user?.bookings as CarBooking[])
    }
    if (!bookingLoading) {
      setAllBookingsData(allBookings?.data as BookingsResponse[])
    }
    const myCarBooking = allBookingsData?.filter(
      (booking: BookingsResponse) => {
        return booking.car?.ownerId === user?.id
      }
    )
                  ...
                  <div className="w-full border shadow-xl rounded-t-xl">
                    ...
                    <div className="px-3">
                      <div className="border-b flex justify-between py-2">
                        ...
                        <p>{CarBooking.car.name}</p>
                      </div>
                      <div className="border-b flex justify-between py-2">
                        <p className="font-bold">Location:</p>
                        <p>{CarBooking.car.city}</p>
                      </div>
                      <div className="border-b flex justify-between py-2">
                        <p className="font-bold">Transmission:</p>
                        <p>{CarBooking.car.carTransmission}</p>
                      </div>
                      <div className="border-b flex justify-between py-2">
                        <p className="font-bold">Rent Per Day:</p>
                        <p>{CarBooking.car.rentPerDay}</p>
                      </div>
                      <div className="border-b flex justify-between py-2">
                    ...
                    {CarBooking.status === "Confirmed" && (
                      <div className="py-3 px-3 flex gap-3">
                        <Button
                          onClick={() => {
                            completeBooking({ id: CarBooking.id }).then(
                              (res) => res.data && setRatingPopup(true)
                            )
                ...
                >
                  <div className="w-full border shadow-xl rounded-t-xl">
                    ...
                    <div className="px-3">
                      ...
                      <div className="border-b flex justify-between py-2">
                        <p className="font-bold">Booking for:</p>
                        <p>{CarBooking.car.name}</p>
                      </div>
                      <div className="border-b flex justify-between py-2">
                        <p className="font-bold">Rent Per Day:</p>
                      ...
                      <div className="border-b flex justify-between py-2">
                        <p className="font-bold">Car Reg#:</p>
                        <p>{CarBooking.car.registrationNo}</p>
                      </div>
                      <div className="border-b flex justify-between py-2">
                        <p className="font-bold">Booking Date:</p>
                        <p>{format(CarBooking.bookingDate, "PPP")}</p>
