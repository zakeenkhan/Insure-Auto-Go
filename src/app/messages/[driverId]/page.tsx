...
import { useSentMessageMutation } from "@/lib/features/conversation/conversation-with-user"
import { set } from "date-fns"
type Props = {
  params: { driverId: number }
}
export default function Messages({ params }: Props) {
  console.log(params.driverId, "driverId")
  ...
  //   sent Message
  const [sentMessage, { isLoading: sentMessageLoading }] =
  ...
  useEffect(() => {
    sentMessage({
      driverId: Number(params.driverId),
    }).then((res) => {
      res?.error?.data?.error?.message[0] === "Conversation already exists"
        ? setSelectedConversationId(`${user?.id}-${params.driverId}`)
        : setSelectedConversationId(`${res.data?.userId}-${res.data?.driverId}`)
    //   console.log(res?.error?.data?.error?.message[0], "res")
    })
