...
export class UserSeederService implements OnModuleInit {
  ...
  async seedUsers() {
    ...
    if (usersCount === 0) {
      ...
      // Driver (as a User + Driver profile)
      ...
      const car1 = await this.prisma.car.create({
        data: {
          ...
          ratingCount: 2,
          weightage: new Prisma.Decimal(0),
        },
      });
      ...
      const car2 = await this.prisma.car.create({
        data: {
          ...
          averageRating: new Prisma.Decimal(4.2),
          ratingCount: 3,
          weightage: new Prisma.Decimal(0),
        },
      });

      // Bookings (one with driver, one without)
      ...
      // Conversation between user and driver
